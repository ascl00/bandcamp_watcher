...
int check_bandcamp_files(const char *path, band_info_t *band_info)
{
    ...
    size_t type_counts[3] = {0};  // Track counts for up to 3 types we check
    const char *types[] = {"flac", "aac", "m4a"};
    int num_types = 3;
    int found_type = -1;
    ...
    while ((de = readdir(d)) != NULL)
    {
        ...
        for (int i = 0; i < num_types; i++) {
            char ext_pattern[32];
            snprintf(ext_pattern, sizeof(ext_pattern), ".%s", types[i]);
    ...
    strcpy(band_info->file_type, types[found_type]);
